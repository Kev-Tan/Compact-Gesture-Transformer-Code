...
def generate_csv(args, split):
    ...
    for index, (id, path) in enumerate(DataLoader(dataset_obj)):
        dic_target_img_arr.append({'class_id': id.item(), 'dir': path})
    ...
    try:
        with open('D:\School\Lab\Compact-Gesture-Transformer-Code\dataset_generation\data_utils\output.json', 'w') as json_file:
            s = json.dump(dic_target_img_arr, json_file, indent=4) # Using indent for human-readable output
            print(s)
        print("Successfully wrote the array to output.json")
    except IOError as e:
